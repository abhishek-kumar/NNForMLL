...
BRSingleLayerNN::BRSingleLayerNN(io & fileio, int numFeatures, int numHiddenUnits, int numTags) :
	p(numFeatures), d(numHiddenUnits), k(numTags), xtr(fileio.xtr), ytr(fileio.ytr), m(xtr.size()), ytr_tag(k, data_t()), baseModels()  { 
