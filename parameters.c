...
void printTagCorrelations(parameters & p, int n)
{
	...
	for(int h=0; h<(p.d+1); ++h)
	{
		...
		cout << "Top Tags for hidden node number " << h << endl << "\t";
		for(int i=0;i<n;++i) cout << "(" << weights[i].second << ", " << p.val(1,h,weights[i].second) << "), ";
		cout << endl;
	}
	for(int h=0; h<(p.d+1); ++h)
	{
		...
		cout << "Top words for hidden node number " << h << endl << "\t";

		for(int i=0;i<n;++i) cout << "(" << weights[i].second << ", " << p.val(0,weights[i].second, h) << "), ";
